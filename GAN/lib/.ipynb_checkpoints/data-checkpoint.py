...
def get_traffic_dataset(data_type, with_vol=True):
    """
    Get different returns series.
    """
#     df = pd.read_csv(f'/home/dsail/migyeongk/기초연구실/minji/data/{data_type}_0wnd_40data.csv') 

#     data_raw = df[['traffic']].values.reshape(1, -1, 1)
#     data_raw = torch.from_numpy(data_raw).float()
#     pipeline = Pipeline(steps=[('standard_scale', StandardScalerTS(axis=(0, 1)))])
#     data_preprocessed = pipeline.transform(data_raw)
#     return pipeline, data_raw, data_preprocessed

    df = pd.read_csv(f'/home/dsail/migyeongk/기초연구실/minji/github/Conditional-Sig-Wasserstein-GANs/data/network/{data_type}_10wnd_40data_new.csv') 

    data_raw = df.values.reshape(1, -1, len(df.columns))
    data_raw = torch.from_numpy(data_raw).float()
    pipeline = Pipeline(steps=[('standard_scale', StandardScalerTS(axis=(0, 1)))])
    data_preprocessed = pipeline.transform(data_raw)
...
def get_traffic_emb_dataset(data_type, with_vol=True):
    """
    Get different returns series.
    """

#     df = pd.read_csv(f'/home/dsail/migyeongk/기초연구실/minji/github/Conditional-Sig-Wasserstein-GANs/data/network/{data_type}_10wnd_40data_new_emb_datalen2.csv')
#     data_raw = df.values.reshape(1, -1, len(df.columns))
#     data_raw = torch.from_numpy(data_raw).float()
#     pipeline = Pipeline(steps=[('standard_scale', StandardScalerTS(axis=(0, 1)))])
#     data_preprocessed = pipeline.transform(data_raw)
...
#     return pipeline, data_raw, data_preprocessed
    ...
    df = np.load(f'/home/dsail/migyeongk/기초연구실/minji/github/Conditional-Sig-Wasserstein-GANs/data/network/{data_type}_10wnd_40data_new_emb_256.csv.npy')
    
    data_raw = df.reshape(256, -1, 11)
    data_raw = torch.from_numpy(data_raw).float()
    pipeline = Pipeline(steps=[('standard_scale', StandardScalerTS(axis=(0, 1)))])
    data_preprocessed = pipeline.transform(data_raw)
...
def get_geant_abilene_dataset(data_type, with_vol=True):
    """
    Get different returns series.
    """
#     df = pd.read_csv(f'/home/dsail/migyeongk/기초연구실/minji/data/{data_type}_0wnd_40data.csv') 
...
#     return pipeline, data_raw, data_preprocessed
    ...
    if data_type == 'GEANT':
        df = pd.read_csv(f'/home/dsail/migyeongk/기초연구실/minji/github/Conditional-Sig-Wasserstein-GANs/data/GEANT/geant_windowing.csv')
    elif data_type == 'Abilene':
        df = pd.read_csv(f'/home/dsail/migyeongk/기초연구실/minji/github/Conditional-Sig-Wasserstein-GANs/data/Abilene/abilene_windowing.csv')
...
def get_geant_abilene_emb_dataset(data_type, with_vol=True):
    """
    Get different returns series.
    """
#     df = pd.read_csv(f'/home/dsail/migyeongk/기초연구실/minji/data/{data_type}_0wnd_40data.csv') 
...
#     return pipeline, data_raw, data_preprocessed
    ...
    print(data_type)
    if data_type == 'GEANT_emb':
        df = pd.read_csv(f'/home/dsail/migyeongk/기초연구실/minji/github/Conditional-Sig-Wasserstein-GANs/data/GEANT/GEANT_emb_10wnd_40data.csv')
    elif data_type == 'Abilene_emb':
        df = pd.read_csv(f'/home/dsail/migyeongk/기초연구실/minji/github/Conditional-Sig-Wasserstein-GANs/data/Abilene/Abilene_emb_10wnd_40data.csv')
...
def get_data(data_type, p, q, **data_params):
    ...
    elif data_type in ['interactive', 'bulk', 'video', 'web']:
        pipeline, x_real_raw, x_real = get_traffic_dataset(data_type)
    elif data_type in ['interactive_emb', 'bulk_emb', 'video_emb', 'web_emb']:
        pipeline, x_real_raw, x_real = get_traffic_emb_dataset(data_type)
    elif data_type in ['GEANT', 'Abilene']:
        pipeline, x_real_raw, x_real = get_geant_abilene_dataset(data_type)
    elif data_type in ['GEANT_emb', 'Abilene_emb']:
        pipeline, x_real_raw, x_real = get_geant_abilene_emb_dataset(data_type)
    else:
        raise NotImplementedError('Dataset %s not valid' % data_type)
    #ssert x_real.shape[0] == 1
