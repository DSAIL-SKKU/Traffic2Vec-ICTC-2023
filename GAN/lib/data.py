...
def get_traffic_dataset(data_type, with_vol=True):
    ...
    df = pd.read_csv('../data/' +data_type+'_10wnd_40data_new.csv') 
...
def get_traffic_emb_dataset(data_type, with_vol=True):
    ...
    data_ = data_type.split('_')[0]
    current_dir = os.getcwd() + '/data/'
    dir = current_dir + data_ + '_10wnd_40data_new_emb_.csv.npy'
    df = np.load(dir)    
    
    data_raw = df.reshape(df.shape[1], df.shape[0], -1)
    data_raw = torch.from_numpy(data_raw).float()
    pipeline = Pipeline(steps=[('standard_scale', StandardScalerTS(axis=(0, 1)))])
    data_preprocessed = pipeline.transform(data_raw)
